...
export default function Profile() {
  ...
  <div className="flex items-center space-x-6">
    ...
    <div>
      <h2 className="text-lg font-semibold flex items-center gap-2">
        <User className="w-5 h-5 text-gray-700 dark:text-gray-300" /> {user.userName}
      </h2>
      <p className="flex items-center gap-2">
        <Mail className="w-5 h-5 text-gray-600 dark:text-gray-400" /> {user.email}
      </p>
      <p className="flex items-center gap-2">
        <Phone className="w-5 h-5 text-gray-600 dark:text-gray-400" /> {user.phone}
      </p>
      <p className="flex items-center gap-2">⚧ {user.gender}</p>
      <p className="flex items-center gap-2">
        <Calendar className="w-5 h-5 text-gray-600 dark:text-gray-400" /> {new Date(user.DOB).toLocaleDateString()}
      </p>
    </div>
  </div>
  ...
  <div className="mt-4 bg-gray-100 dark:bg-gray-700 p-3 rounded-lg shadow-sm text-center">
    <p className="font-medium flex items-center justify-center gap-2">
      <Calendar className="w-5 h-5 text-gray-600 dark:text-gray-400" /> Joined: {new Date(user.createdAt).toLocaleDateString()}
    </p>
  </div>
...
<div className="mt-10 p-4 bg-gray-50 dark:bg-gray-800 shadow-md rounded-lg">
  <div className="flex justify-between items-center">
    <h1 className="text-lg font-semibold">
      {numOfAllMessages > 0 ? `Messages (${numOfAllMessages})` : "No messages"}
    </h1>
    {numOfAllMessages > 0 && (
      <button
        type="reset"
      ...
      >
        <Trash2 className="w-5 h-5" /> Delete All {isLoading && <LoaderCircle className="animate-spin ml-2" />}
      </button>
    )}
  </div>
  ...
  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
    {messages
      ?.slice()
      .sort((a, b) =>
        a.pin === b.pin ? new Date(b.createdAt) - new Date(a.createdAt) : b.pin - a.pin
      )
      .map((message, index) => (
        <Message
          key={index}
